 ...
 * Change-log:
 * 	11/25/2017
 * 	-fixed double execution of "What would you like to do?" on first round of game
 * 
 * 	11/5/17
 * 	-overwrote super toString() for grammar
 * 	-removed todo for creatures moving on their own, since I guess they are not described as moving
 * 	-removed criticalError() since it messes with creatures cleaning in other rooms
 * 
 * 	10/30/17 
 * 	-added to the default case for inputs.size()==1 so that it checks for inputs with a colon
 * 
 * 	10/23/17
 * 	-updated for room's linkedList
 * 
 * 	10/16/17
...
public class PC extends Creature{
	...
	public String toString() {
		return (this.getName() + ", " + this.getDesc()+", a "+this.type());
	}
...
/*	private static boolean criticalError() {
	...
	/**
	 * Finds a colon in the input string.
	 * @param s the string
	 * @return true if colon is present
	 */
	private boolean hasColon(String s) {
	...
	/**
	 * Returns the first half of the input before the colon
	 * @param s
	 * @return
	 */
	...
	/**
	 * Returns the second half of the input after the colon.
	 * @param s
	 * @return
	 */
	...
	public void play(Scanner s) {
		...
		String commandList = 
				...
				"'NAME':CLEAN - If the creature with 'NAME' exists in the room,\n"+
				"if it is an animal, it will clean the room."+
				"'NAME':DIRTY - If the creature with 'NAME' exists in the room,\n"+
				"if it is an NPC, it will dirty the room."+
				"'NAME':NORTH - Moves the creature with 'NAME' to the north.\n"+
				"'NAME':SOUTH - Moves the creature with 'NAME' to the south.\n"+
				"'NAME':EAST - Moves the creature with 'NAME' to the east.\n"+
				"'NAME':WEST - Moves the creature with 'NAME' to the west.\n"+
...
//----------------------------------------------------------------------------------------------------------cheat codes here
							...
							case "CHEAT8":{
								for(Creature c: this.getRoom().getList()) {
									if(!c.isPlayer()) {
										System.out.println(c.getName()+" was brutally murdered.");
										this.getRoom().removeCreature(c);
									}
								}
...
//-----------------------------------------------------------------------------------------------------------------regular commands con't
							default:{
								...
								if(this.hasColon(inputs.get(0))) {
									String firstHalf = this.firstHalf(inputs.get(0)).toUpperCase();
									String secondHalf = this.secondHalf(inputs.get(0)).toUpperCase();
									switch (secondHalf) {
										case "CLEAN":{
											if(this.getRoom().findCreature(firstHalf)!=null) {
												if(this.getRoom().findCreature(firstHalf).type().equals("ANIMAL")) {
													this.getRoom().findCreature(firstHalf).modifyRoom();	
												}
												else {
													System.out.println(this.getRoom().findCreature(firstHalf).getName()+" did not clean the room.");
												}
											}
											break;
										}
										case "DIRTY":{
											if(this.getRoom().findCreature(firstHalf)!=null) {
												if(this.getRoom().findCreature(firstHalf).type().equals("NPC")) {
													this.getRoom().findCreature(firstHalf).modifyRoom();	
												}
												else {
													System.out.println(this.getRoom().findCreature(firstHalf).getName()+" did not dirty the room.");
												}
											}
											break;
										}
										case "NORTH":{
											if(this.getRoom().findCreature(firstHalf)!=null) {
												if(this.getRoom().getRoomNorth()==null) {
													System.out.println("There is no room to the north.");
												}
												else {
													this.getRoom().getRoomNorth().addCreature(this.getRoom().findCreature(firstHalf));		
												}
											}
											else {
										...
										case "SOUTH":{
											if(this.getRoom().findCreature(firstHalf)!=null) {
												if(this.getRoom().getRoomSouth()==null) {
													System.out.println("There is no room to the south.");
												}
												else {
													this.getRoom().getRoomSouth().addCreature(this.getRoom().findCreature(firstHalf));		
												}
											}
											else {
												System.out.println("That creature does not exist in this room.");
										...
										case "EAST":{
											if(this.getRoom().findCreature(firstHalf)!=null) {
												if(this.getRoom().getRoomEast()==null) {
													System.out.println("There is no room to the east.");
												}
												else {
													this.getRoom().getRoomEast().addCreature(this.getRoom().findCreature(firstHalf));		
												}
											}
											else {
												System.out.println("That creature does not exist in this room.");
										...
										case "WEST":{
											if(this.getRoom().findCreature(firstHalf)!=null) {
												if(this.getRoom().getRoomWest()==null) {
													System.out.println("There is no room to the west.");
												}
												else {
													this.getRoom().getRoomWest().addCreature(this.getRoom().findCreature(firstHalf));		
												}
											}
											else {
												System.out.println("That creature does not exist in this room.");
								...
								//otherwise, we've exhausted all of our legal commands
								else if(inputs.get(0).equals("") && firstRound==true) {
									firstRound=false;
								}
								else {
